...
function makeShard(corrupted) {
  ...
  const pad = new THREE.Mesh(
    ...
    mat(corrupted ? COLORS.bad : COLORS.shard, { transparent: true, opacity: 0.45 }),
  );
  pad.rotation.x = -Math.PI / 2;
  pad.position.y = -0.72;
  root.add(mesh, pad);
  root.userData.crystal = mesh;
  return root;
}
...
function buildUi() {
  ...
  ui.copy = el("p", {}, "The island is dissolving. Walk into the cyan crystals, feed the yellow pylon, then keep going.");
...
const pylonSpots = [
  { x: 0.2, z: 17.2, color: COLORS.yellow, name: "Sun pylon" },
...
function setMode(mode) {
  ...
  if (mode === "start") {
    ...
    ui.copy.textContent = "The island is dissolving. Walk into the cyan crystals, feed the yellow pylon, then keep going.";
...
function updateStorm(dt) {
  ...
  for (const shard of shards) {
    ...
    if (Math.hypot(shard.mesh.position.x - storm.position.x, shard.mesh.position.z - storm.position.z) < 4.6) {
      ...
      const crystal = shard.mesh.userData.crystal;
      crystal.material.color.setHex(COLORS.bad);
      crystal.material.emissive.setHex(COLORS.bad);
    }
  }
...
function updatePlayer(dt) {
  ...
  if (pylon && !pylon.restored) {
    ...
    if (input.action) {
      state.talked = true;
      showSpeech("Five shards each. Skip the purple ones — storms poison them.", 4.5);
      sfx("talk");
    }
  } else {
...
function decayRate() {
  let rate = 0.52;
  if (state.time > 28) rate = 0.85;
  if (state.time > 55) rate = 1.25;
  if (state.time > 80) rate = 1.7;
  rate -= state.restored * 0.28;
  return Math.max(0.22, rate);
}
...
function frame() {
  ...
  for (const shard of shards) {
    if (shard.taken) continue;
    shard.mesh.userData.crystal.rotation.y += dt * 2.2;
    shard.mesh.position.y = heightAt(shard.x, shard.z) + 0.9 + Math.sin(clock.elapsedTime * 2 + shard.bob) * 0.16;
  }
