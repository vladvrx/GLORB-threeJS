  ...
  if (fs.existsSync(file)) {
    ...
    if (start >= 0 && end > start) {
      try {
        ...
        // keep an empty overlay and rewrite
      }
    }
  }
  for (const [key, scene] of Object.entries(scenes)) {
    const id = sceneIdFromPackKey(key);
    if (id !== "IslandWest" && id !== "IslandIntro") continue;
    overlay[id] = scene;
    overlay[`Scene_${id}`] = scene;
  }
...
window.__STUDIO_APPLY__=function(id,manifest,runtime,phase){
  if((id==="IslandWest"||id==="Scene_IslandWest")&&!window.__GLORB_STUDIO__)return;
...
export function applyPack(root, pack) {
  ...
  if (pack.scenes) {
    ...
    for (const [key, scene] of Object.entries(pack.scenes)) {
      const id = sceneIdFromPackKey(key);
      if (id !== "IslandWest" && id !== "IslandIntro") continue;
      scenes[`Scene_${id}`] = shippedScene(scene);
    }
    pack.scenes = scenes;
  }
...
if (isMain) {
  ...
  (async () => {
    ...
    if (packPath) {
      ...
      console.error("Usage: node scripts/apply-to-datab-each.mjs --root /workspace --pack pack.json [--ensure] [--start]");
      process.exit(1);
    }
    if (flag("--start")) {
