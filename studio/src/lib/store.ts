...
import { ACTOR_ASSETS, cloneObject, mergeShippedAnchors, newProp, projectFromBundle, slimStudioProject } from "@/lib/project";
...
const SAVE_KEY = "glorb-studio-maps-v2";
...
export const useEditor = create<EditorState>((set, get) => {
  ...
  return {
        ...
        if (saved) {
          try {
            ...
            if (parsed.format === "datab-each-studio-v1" && parsed.scenes) {
              project = slimStudioProject(mergeShippedAnchors(parsed, bundle), bundle);
            }
          } catch {
            // keep shipped island
          }
        }
        project = slimStudioProject(project, bundle);
    ...
    exportStudio: () => {
      const { project } = get();
      if (!project) return;
      download("glorb-studio.json", JSON.stringify(project, null, 2));
    },
    exportGamePack: () => {
      const { project } = get();
      if (!project) return;
      download("glorb-game-pack.json", JSON.stringify(exportGamePack(project), null, 2));
    },
    async importStudio(file) {
      ...
      set({
        project: slimStudioProject(mergeShippedAnchors(parsed, bundleCache), bundleCache),
        selectedId: null,
        dirty: true,
        past: [],
        future: [],
    ...
    async applyToGame() {
      ...
      return { gameUrl: payload.gameUrl ?? "http://127.0.0.1:43219/three-js" };
    },
