...
import { GLORB_ISLAND, isPaintSquare } from "@/lib/maps";
...
function makePaintSquareTerrain() {
  const { cx, cz, rx, rz, floorY, thickness } = GLORB_ISLAND;
  const width = rx * 2;
  const depth = rz * 2;
  ...
  mesh.position.set(cx, floorY - height / 2, cz);
...
export class ViewportEngine {
  ...
  private async loadTerrain(scene: SceneData, token: number) {
    try {
      const terrain = isPaintSquare(scene) || scene.procedural === "paint-square" || !scene.glb
        ? makePaintSquareTerrain()
        : await this.library.loadSceneBase(scene.glb);
      ...
      if (this.disposed || this.syncToken !== token || this.sceneId !== scene.id) return;
      if (this.terrain) this.island.remove(this.terrain);
      this.terrain = makePaintSquareTerrain();
